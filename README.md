...
| --- | --- | --- | --- |
| `maxAge` | `number` | — (required) | Freshness window at the edge, in seconds |
| `staleWhileRevalidate` | `number \| 'unbounded'` | — | SWR window. `'unbounded'` resolves to 31,536,000 s (1 year) |
...
- **Automatic route tag** `route:/blog/:id` from `c.req.routePath` enables purging per route template; skipped for the bare `/*` pattern.
